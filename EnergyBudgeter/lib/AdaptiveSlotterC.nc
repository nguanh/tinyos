 ...
 */
generic configuration AdaptiveSlotterC(uint8_t NUM_SLOTS, uint16_t BASE_INTVL, uint16_t CYCLE_LEN, uint8_t ALPHA) {
  provides {
    interface Slotter;
  }
  uses {
...
implementation {
  components new AdaptiveSlotterP(NUM_SLOTS, BASE_INTVL, CYCLE_LEN, ALPHA); 
  Slotter         = AdaptiveSlotterP;
  AveragingSensor = AdaptiveSlotterP;

  components MainC;
  MainC -> AdaptiveSlotterP.Init;

  // configure job functionality
  components new EAJobC() as Job;
  components new EAPeriodicJobConfigC() as JobConfig;
  AdaptiveSlotterP       -> Job.Job;
  Job.JobConfig          -> JobConfig;
  JobConfig.SubJobConfig -> AdaptiveSlotterP;
}
